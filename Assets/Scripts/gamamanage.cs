...
public class gamamanage : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown("t")) //시험용으로 만들어놓은 부분입니다. t를 누르면 전구를 한개 넣는다고 생각해주세요!
        {
           bulbcount++;

            if (bulbcount == 1) onecount();
            else if(bulbcount == 2) twocount();
            else if(bulbcount == 3) threecount();
            else if (bulbcount == 4) fourcount();
            else if (bulbcount == 5) fivecount();
            else if (bulbcount == 6) sixcount();
        }
    }
    ...
    private void fourcount()
    {
        ...
        Instantiate(shotgun, new Vector3(0,1f,0), Quaternion.identity);
